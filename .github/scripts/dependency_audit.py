...
def parse_npm_audit(data: Any) -> list[Finding]:
    ...
    for package, raw_vuln in vulnerabilities.items():
        if not isinstance(package, str):
            raise InvalidScannerEvidence("npm-audit: nome de pacote inválido")
        ...
        advisory_ids: list[tuple[str, str]] = []
        for item in via:
            if isinstance(item, str):
                advisory_ids.append((item, severity))
                ...
                advisory_ids.append((str(advisory_id), str(item.get("severity") or severity).lower()))
            else:
                raise InvalidScannerEvidence(f"npm-audit: via inválido ({package})")
        if not advisory_ids:
            advisory_ids.append((f"npm:{package}:{vuln.get('range', 'unknown')}", severity))
        for advisory_id, advisory_severity in advisory_ids:
            findings.append(
                Finding(
                    scanner="npm-audit",
                    package=package,
                    installed_version=str(vuln.get("range") or "unknown"),
...
def analyze_scanner(scanner: str, raw_text: str, metadata: dict[str, Any]) -> ScannerResult:
    ...
    if data is not None:
        try:
            findings = parse_pip_audit(data) if scanner == "pip-audit" else parse_npm_audit(data)
    ...
    normalized = tuple(sorted(unique.values(), key=lambda item: item.fingerprint))
    state = VALID_WITH_FINDINGS if normalized else VALID_NO_FINDINGS
    return ScannerResult(scanner, state, exit_code, normalized, ())


def analyze(evidence: dict[str, tuple[str, dict[str, Any]]]) -> dict[str, Any]:
    scanner_results = [
        analyze_scanner(scanner, raw_text, metadata)
...
def render_findings(findings: list[dict[str, Any]], *, fixable: bool) -> str:
    ...
    for item in selected:
        ...
        lines.append(
            ...
            f"{qualifier} — fingerprint `{item['fingerprint']}`"
        )
    return "\n".join(lines) + ("\n" if lines else "")
