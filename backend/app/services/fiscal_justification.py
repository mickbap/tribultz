...
O conteúdo é determinístico (texto estático); não há custo de IA/tokens.
"""
...
class FiscalJustification(TypedDict):
    base_legal: str
    explicacao: str
    correcao: str


FISCAL_JUSTIFICATIONS: dict[str, FiscalJustification] = {
    "XML_PARSE": {
    ...
    "CST_GROUP_MATCH": {
        "base_legal": "NT 2025.002-RTC, Regra de Coerência de Grupo: CST deve corresponder ao grupo XML utilizado",
        "explicacao": (
    ...
    "CST_SEMANTIC": {
        "base_legal": "LC 214/2025, Art. 9º, §6º — Vedação de tributação em operações imunes ou isentas",
        "explicacao": (
    ...
    "LAYOUT_NFE": {
        "base_legal": (
            "NT 2015.003 e atualizações — Leiaute NF-e v4.0; "
            "NT 2025.002-RTC — Campos IBS/CBS adicionados ao leiaute"
        ),
        "explicacao": (
...
# Plans that have access to technical justifications
JUSTIFICATION_PLANS = frozenset({"profissional", "empresarial", "contador"})


def get_justification(rule_id: str) -> FiscalJustification | None:
    """Return justification dict for a rule_id, or None if not mapped."""
    return FISCAL_JUSTIFICATIONS.get(rule_id)
...
def enrich_findings(
    ...
    """
    Enrich each finding with justification data based on plan access.

    - If plan_slug is in JUSTIFICATION_PLANS → attach full justification
    - Otherwise → set justification_gated=True (frontend shows upgrade CTA)
    """
    has_access = plan_slug in JUSTIFICATION_PLANS
    enriched = []
    for f in findings:
        finding = dict(f)
        rule_id = finding.get("rule_id", "")
        justification = get_justification(rule_id)

        if justification:
            if has_access:
                finding["justification"] = justification
                finding["justification_gated"] = False
            else:
                finding["justification_gated"] = True
        else:
            finding["justification_gated"] = False
